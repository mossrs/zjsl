...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      submitForm(formName) {
        // this.$refs.formName   --> document.getElementById('ruleForm')
        this.$refs[formName].validate(valid => {
          ...
          this.$axios.post(this.host+'/mg_admin/login/',this.ruleForm,{
            headers:
                    {
    ...
    mounted() {
      ...
      let step = 0;
      // 定义三条不同波浪的颜色
      let lines = ["rgba(0,222,255, 0.2)", "rgba(157,192,249, 0.2)", "rgba(0,168,255, 0.4)"];
